...
namespace Abstracta.JmeterDsl
{
    ...
    public static class JmeterDsl
    {
        ...
        /// <br/>
        /// JMeter generates <c>__jm__while__idx</c> variable containing the iteration number (0 indexed),
        /// which can be helpful in some scenarios.
        /// </summary>
        /// <param name="condition">contains an expression that will be evaluated to identify when to stop looping.</param>
        /// <param name="children">contains the test plan elements to execute while the condition is true.</param>
        /// <returns>the controller instance for further configuration and usage.</returns>
        /// <seealso cref="Core.Controllers.WhileController"/>
        public static WhileController WhileController(string condition, params IThreadGroupChild[] children) =>
            WhileController(null, condition, children);

        /// <summary>
        /// Same as <see cref="WhileController(string, IThreadGroupChild[])"/> but allowing to set a name which
        /// defines autogenerated variable created by JMeter containing iteration index.
        /// </summary>
        /// <param name="name">specifies the name to assign to the controller. This variable affects the JMeter autogenerated variable <c>__jm__&lt;controllerName&gt;__idx</c> which holds the loop iteration number (starting at 0).</param>
        /// <param name="condition">contains an expression that will be evaluated to identify when to stop looping.</param>
        ...
        /// <returns>the controller instance for further configuration and usage.</returns>
        /// <seealso cref="Core.Controllers.WhileController"/>
        /// <seealso cref="WhileController(string, IThreadGroupChild[])"/>
        public static WhileController WhileController(string name, string condition, params IThreadGroupChild[] children) =>
            new WhileController(name, condition, children);
